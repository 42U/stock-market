...
class PREPDATA:
    ...
    def getData(self, tf: str="4h") -> list:
        ...
        # Retrieve all data
        data = client.klines(self.symbol, tf, limit=2)

        # set most recent bar data
        bar = data[-1]
        self.date = str(bar[0])
...
if __name__ == '__main__':
    ...
    # print(coin_data)
    testvs = ticker.pullfromDB(db_table)[-1][1]
    # Check the current time in ms
    currtime = str(time.time()*1000)
    # Insert results into the database 1 bar at a time
    for bar in coin_data:
       ...
       if currtime >= str(bar[6]) and testvs != str(bar[6]):
